<?php
    ...
    $aConsumo= ConsultarDados("consumo", "cdrese", $chave, false, false, $cdclie);
            ...
            <!--div class="col-lg-12"-->
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        ...
                        <?php $titu = "Demonstrativo de Consumo - Reserva No.: ".$chave;?>
                        <button type="button" class="btn btn-primary btn-lg btn-block"><i
                                                    class="fa fa-shopping-cart"></i> <?php echo $titu;?>
                        </button>
                    </div>
                    <div class="ibox-content">
                        <!--form class="form-horizontal" method="POST" enctype="multipart/form-data" action="usuariosa.php"-->
                            <div class="pull-left">
                                <a onclick="#" href="consumo.php" class="btn btn-warning">Retornar</a>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-example" >
                                    ...
                                    <tbody>
                                        <?php $flcabe=false; ?>
                                        <?php $total=0; ?>
                                        <?php for ($x =0; $x <= (count($aConsumo)-1); $x++) { ?>
                                            <?php $total = $total + $aConsumo[$x]["vlcons"]; ?>
                                        <?php }?>
                                        <?php for ($f =0; $f <= (count($aConsumo)-1); $f++) { ?>
                                            <tr class="gradeX">
                                                <?php if ($flcabe == false) { ?>
                                                    <?php $coluna1 = "RESERVA: ".$aConsumo[0]["cdrese"]; ?>
                                                    <!--busca o hospede no cadastro de reservas-->
                                                    <?php $aRese= ConsultarDados("reservas", "cdrese", $aConsumo[0]["cdrese"], false, false,$cdclie); ?>
                                                    <?php If (count($aRese) > 0) {?>
                                                        <?php $cdhosp = $aRese[0]["cdhosp"] ; ?>
                                                    <?php } Else {?>
                                                        <?php $cdhosp = "00000000000" ;?>
                                                    <?php }?>

                                                    <!--busca o nome dohospede no cadastro de hospedes-->
                                                    <?php $aHosp= ConsultarDados("hospedes", "cdhosp", $cdhosp, false, false,$cdclie); ?>
                                                    <?php If (count($aHosp) > 0) {?>
                                                        <?php $dehosp = substr($aHosp[0]["dehosp"], 0,25) ; ?>
                                                    <?php } Else {?>
                                                        <?php $dehosp = "Hóspede Desconhecido" ;?>
                                                    <?php }?>
                                                    <?php $coluna2 = "HÓSPEDE: ".$cdhosp." - ".$dehosp; ?>
                                                <?php } ?>
                                                <?php $flcons = $aConsumo[$f]["flcons"] ;?>
                                                <?php $preco = number_format($aConsumo[$f]["vlcons"],2,',','.') ;?>
                                                <?php $data = $aConsumo[$f]["dtcons"] ;?>

                                                <?php if ($flcons == 'P') {?>
                                                    <?php $aProd= ConsultarDados("produtos", "cdprod", $aConsumo[$f]["cdcons"], false, false,$cdclie); ?>
                                                    <?php If (count($aProd) > 0) {?>
                                                        <?php $decons = substr($aProd[0]["deprod"], 0,25); ?>
                                                    <?php } Else {?>
                                                        <?php $decons = "Nome Desconhecido" ;?>
                                                    <?php }?>
                                                <?php } Else {?>
                                                    <?php $aServ= ConsultarDados("servicos", "cdserv", $aConsumo[$f]["cdcons"], false, false,$cdclie); ?>
                                                    <?php If (count($aServ) > 0) {?>
                                                        <?php $decons = substr($aServ[0]["deserv"], 0,25); ?>
                                                    <?php } Else {?>
                                                        <?php $decons = "Nome Desconhecido" ;?>
                                                    <?php }?>
                                                <?php }?>
                                                ...
                                                <?php if ($flcabe == false) {?>
                                                    <?php $coluna1 = "Hóspede: ".$cdhosp." - ".$dehosp; ?>
                                                    <?php $coluna2 = "Reserva: ".$aConsumo[$f]["cdrese"]; ?>
                                                    <?php $coluna3 = ""; ?>
                                                    <?php $coluna4 = "Total"; ?>
                                                    <?php $coluna5 = number_format($total,2,',','.'); ?>
                                                    <?php $flcabe = true;?>
                                                    <?php $f-- ;?>
                                                <?php } Else {?>
                                                    <!--?php $coluna1 = $aConsumo[$f]["nrsequ"]; ?-->
                                                    <?php $coluna1 = $aConsumo[$f]["nrsequ"]; ?>
                                                    <?php $coluna2 = date("d/m/Y H:i:s",strtotime($data)); ?>
                                                    <?php $coluna3 = $aConsumo[$f]["cdcons"]." - ".$decons; ?>
                                                    <?php $coluna4 = $aConsumo[$f]["qtcons"]; ?>
                                                    <?php $coluna5 = $preco; ?>
                                                <?php }?>
