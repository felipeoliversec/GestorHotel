<?php 	
	...
	if (count($alocais) > 0 and $Acao == "edita"){
		...
		if ($qthosp > $qtloca) {
			$demens = "Quantidade de hóspedes superior à permitida no Quarto/Suíte/Chalé!";
	...
	if ($Flag == true and $Acao == "edita") {
		...
		if (empty($Tem) == false) {
			if (trim($Tem) !== $cdrese) {
				$demens = "Já existe uma reserva no: ".trim($Tem)." para esse Quarto/Suíte/Chalé!";
				$detitu = "Weber Hotel&copy; | Cadastro de Reservas";
				header('Location: mensagem.php?demens='.$demens.'&detitu='.$detitu);
				$Flag=false;				
			}
	...
	if ($Flag == true) {
		...
		switch (get_post_action('edita','apaga','pagar')) {
	    case 'edita':
	    ...
	    case 'pagar':
			$demens = "Fechamento/Pagamento efetuada com sucesso. Imprima a comanda/cupom em Registrar Consumo!";
		...
		$detitu = "Weber Hotel&copy; | Cadastro de Reservas";
		$devolt = "reservasq.php?chave=".$cdloca;
